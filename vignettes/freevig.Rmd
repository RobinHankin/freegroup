...
Then to convert to a more useful form we would have
...
The representation for R object `x` is still the two-row matrix form,
but the print method is active and uses a more visually appealing
scheme.

We can concatenate `x` with itself by using "`+`":
