...
The free group is an interesting and instructive mathematical This
package provides some functionality for manipulating the free group on
a finite list of symbols~\citep{wikipedia2019}.  Informally, the {\em
free group} $\left(X,\circ\right)$ on a set $S=\{a,b,c,\ldots,z\}$ is
the set $X$ of {\em words} that are objects like
...
appealing scheme.  We can coerce strings to free objects:
...
The \pkg{freegroup} furnishes a consistent and documented suite of
reasonably efficient \proglang{R}-centric funcationality in the form
of a package.  Further work might include the finitely presented
groups but it is not clear whether this would be consistent with the
precepts of \proglang{R}.
