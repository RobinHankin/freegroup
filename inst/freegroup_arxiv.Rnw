...
many concepts of elementary group theory.  The {\tt freegroup} package
...
Sage~\citep{sagemath2019}, and {\tt sympy}~\citep{sympy2017}
although in those systems the emphasis is on finitely presented
groups, not in scope for the {\tt freegroup} package.  There are also
...
appealing scheme.  The default alphabet used is {\tt letters}.  We
...
However, note that the ``{\tt +}'' symbol is usually reserved for
commutative and associative operations; string juxtaposition is
associative.  Multiplication by integers---denoted in {\tt freegroup}
idiom by ``{\tt *}''---is also defined.  Suppose we want to
concatenate 5 copies of {\tt x}:
...
Powers are defined as per group conjugation: {\tt x\string^y ==
...
or in package idiom {\tt .[x,y]=-x-y+x+y}:
